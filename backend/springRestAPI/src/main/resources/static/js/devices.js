        ...
        html += `
            <tr class="${rowClass}">
                ...
                <td>
                    ...
                    <button class="btn btn-sm btn-primary" onclick="window.location.href='/device-history?id=${device.device_id}'">
...
function createGatePassForDevice(deviceId, deviceUniqueId) {
    ...
    window.location.href = `/create-gate-pass?device_id=${deviceId}`;
}
...
function generateQRCode(deviceId, deviceUniqueId) {
    ...
    const qrUrl = `${CONFIG.API_BASE_URL}${CONFIG.ENDPOINTS.GENERATE_QR}?device_id=${deviceId}`;
