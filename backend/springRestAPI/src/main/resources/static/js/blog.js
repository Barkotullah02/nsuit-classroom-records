            ...
            if (posts.length === 0) {
                ...
                container.querySelectorAll('.post-card').forEach(card => {
                    card.addEventListener('click', () => {
                        window.location.href = `/blog-post?id=${card.dataset.postId}`;
                    });
                });
            }
    ...
    try {
        ...
        if (response.success && response.data.posts.length > 0) {
            container.innerHTML = response.data.posts.map(post => `
                <div class="popular-post" onclick="window.location.href='/blog-post?id=${post.post_id}'">
        ...
        if (response.success && response.data) {
            ...
            if (['super_admin', 'admin'].includes(currentUser.role)) {
                ...
                adminActions.innerHTML = `
                    <button class="btn btn-secondary" onclick="window.location.href='/blog-admin'">
