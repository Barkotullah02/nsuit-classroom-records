...
async function importDevices(rows, skipDuplicates) {
    ...
    const metadataResult = await Utils.apiRequest(CONFIG.ENDPOINTS.METADATA);
    ...
    for (let i = 0; i < dataRows.length; i++) {
        ...
        if (!type && deviceData.type_name) {
            ...
            const createTypeResult = await Utils.apiRequest(CONFIG.ENDPOINTS.METADATA, {
                method: 'POST',
                body: JSON.stringify({
                    type: 'device_type',
        ...
        if (!brand && deviceData.brand_name) {
            // Create new brand
            const createBrandResult = await Utils.apiRequest(CONFIG.ENDPOINTS.METADATA, {
                method: 'POST',
                body: JSON.stringify({
                    type: 'brand',
    ...
    if (successCount > 0) {
        html += `
            <div style="margin-top: 20px;">
                ...
                <button class="btn btn-secondary" onclick="window.location.href='/devices'">
