...
const INVALID_DATES_ENDPOINT = CONFIG.ENDPOINTS.INVALID_DATES || CONFIG.ENDPOINTS.INVALID_INSTALLATION_DATES;
...
async function loadInvalidInstallations() {
    ...
    if (!INVALID_DATES_ENDPOINT) {
        if (tbody) {
            tbody.innerHTML = '<tr><td colspan="9" class="text-center text-danger">Invalid dates API endpoint is not configured</td></tr>';
        }
        Utils.showAlert('Invalid dates API endpoint is not configured');
        return;
    }

    const result = await Utils.apiRequest(INVALID_DATES_ENDPOINT, {
        method: 'GET'
    });
    ...
    if (result.success) {
        ...
        if (tbody) {
            tbody.innerHTML = '<tr><td colspan="9" class="text-center text-danger">Failed to load invalid installation records</td></tr>';
        }
